<?php
    ...
    class Produk {
        ...
        public function getAllProduk($keyword = null){
            $sql = "SELECT sp.id_produk,kp.nama_kategori, sp.nama_produk, sp.satuan, sp.harga_jual, sp.stok_produk FROM stok_produk sp JOIN kategori_produk kp ON sp.id_kategori = kp.id_kategori";
            if ($keyword) {
        ...
        public function tambahProduk($kategori, $nama, $satuan, $harga_jual, $stok, $datetime){
            $query = $this->conn->prepare("INSERT INTO stok_produk (id_kategori, nama_produk, satuan, harga_jual, stok_produk, created_at) VALUES (:kategori, :nama, :satuan, :hargajual, :stok, :tanggal)");
        ...
        public function updateProduk($id, $kategori, $nama, $satuan, $harga_jual, $stok, $datetime){
            $query = $this->conn->prepare("UPDATE stok_produk SET id_kategori = :kategori, nama_produk = :nama, satuan = :satuan, harga_jual = :hargajual, stok_produk = :stok, created_at = :tanggal WHERE id_produk = :id");
            $query->bindParam(":id", $id);
            $query->bindParam(":kategori",$kategori);
            $query->bindParam(":nama", $nama);
            $query->bindParam(":satuan", $satuan);
